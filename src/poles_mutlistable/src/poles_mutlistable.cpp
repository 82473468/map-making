...
poles_mutlistable::poles_mutlistable(ros::NodeHandle node,
    ...
    sub_tracklets = node.subscribe(topic, 10, &poles_mutlistable::gridCallback, (poles_mutlistable *)this);

    marker_pub = node.advertise<visualization_msgs::MarkerArray>("visualization_marker", 1);

    pub_poles = node.advertise<sensor_msgs::PointCloud2>("rslidar_raw",10);
...
void poles_mutlistable::publishpoles_tree(const ros::Publisher* in_publisher,const pcl::PointCloud<pcl::PointXYZI>::Ptr poles_cloud){
    ...
    output_poles.header.frame_id="rslidar";
    in_publisher->publish(output_poles);
}


void poles_mutlistable::gridCallback(const sensor_msgs::PointCloud2 &msg) {
...
//
    ...
    showcylinder(poles_position);
    publish_points(&pub_poles_position, poles_position);
...
//        std::cout<<"\033[32m the cov_scalar is \033[0m"<<cov_scalar<<"the scalar of points is:"<<scalarPoints<<std::endl;
        ...
        std::cout<<"the long and short size is "<<long_size<<","<<short_size<<std::endl;
        if(long_size>0.4&&long_size/short_size>3) continue;
        poleslabel pole;
...
void poles_mutlistable::showcylinder(pcl::PointCloud<pcl::PointXYZI>::Ptr centers){
...
void poles_mutlistable::publish_points(const ros::Publisher* in_publisher,const pcl::PointCloud<pcl::PointXYZI>::Ptr poles_cloud){
    sensor_msgs::PointCloud2 output_poles;
    pcl::toROSMsg(*poles_cloud,output_poles);
